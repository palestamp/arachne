...
class Action:
    ...
    def from_obj(cls, obj, name=None):
        ...
        obj["args"] = recur(_wrap_expression, obj.get("args", {}))
        if name is not None:
            obj["name"] = name
        if obj.get("options"):
    ...
    async def run(self, context, loop=None):
        ...
        try:
            ...
            return Output(
                ...
                failure_details={
                    "_type": "timeout",
                    "_value": self.options.timeout.label,  # pylint: disable=no-member
                },
            )
...
class Output:
    ...
    def as_obj(self):
        if self.failed:
            return {
                "_failed": True,
                "_details": self.failure_details,
            }
        return {
            "_failed": False,
            "_result": self.data,
            "_took": int(self.took * 1000_000),
        }
